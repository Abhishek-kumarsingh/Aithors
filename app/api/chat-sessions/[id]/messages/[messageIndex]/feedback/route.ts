...
export async function PUT(
  req: NextRequest,
  { params }: { params: { id: string; messageIndex: string } }
) {
  try {
    ...
    const backendUrl = `${process.env.NEXT_PUBLIC_API_BASE_URL || "http://localhost:5000"}/api/chat-sessions/${params.id}/messages/${params.messageIndex}/feedback`;
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: errorData.message || "Failed to update message feedback" },
    ...
    return NextResponse.json(
      { error: error.message || "Failed to update message feedback" },
      { status: 500 }
    );
  }
}